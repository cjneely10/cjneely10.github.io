...
For this blog, I have selected a group of genomes of variable quality and completion, including some redundant genomes. I will use the **PhyloSanity** pipeline to evaluate this subset for high quality, non-redundant genomes, which we will initially define as completion &ge; 90% and contamination &le; 5% via CheckM pipeline. We define non-redundant as either 1) having no genomes in the data set whose percent average nucleotide identity (%ANI) is greater than or equal to 98.5, or 2) having a separate genome share &ge;98.5% ANI, but having the maximum completion percent of the copies.
...
The database summary output is fairly self-explanatory - this table is named "evaluation", which was given to it by the settings in the `BIOMETADB` section of `PhyloSanity.ini`. The number of genomes evaluated corresponds to the number of records. Averages and sample standard deviations are provided for numerical data categories. For text or boolean data categories, frequency values are provided - e.g., the most frequently occurring value, the number of times that value occurred, and the total number of nun-null values in that category.
...
In fact, if we query the results for high quality, non-redundant genomes, we can see that very few genomes passed the combined filtering values:
...
- In the above command, the flag `-q hqnr` is a query, which filters the database's results. In this case, the `hqnr` query was stored in the **BioMetaDB** program for convenience, and expands to `-q "is_complete AND NOT is_contaminated AND is_non_redundant"`.
...
- Notice the structure of this query statement - using the values in the `Database` column of the summary output, we created a filtering statement. We could perform similar queries, such as:
	- `-q "redundant_copies != '[]'"` to get genomes with redundant copies
	- `-q "completion > '90.0'"`
...
Notice that this run completes within a few seconds. Here lies one of the benefits of **MetaSanity** - since we are only adjusting a filtering criteria, rerunning the pipeline occurs really quickly. As a counter example, if we were to delete the folder `checkm_results`, then the pipeline would need to rerun this step, which will take more time to complete.

With the new criteria, the number of genomes that are deemed 'complete' has increased.
...
Great! Let's output these sequences to a separate folder - in this case, I will name it `hqnr` - and focus our annotation pipeline on this subset.
